...
const OurServicesList = () => {
    ...
    return (
        ...
        <ul className='lg:max-w-[22rem] sm:max-w-[20rem] sm:p-0 px-1 md:space-y-2 space-y-1'>
            {[
                new Service(0, 'Pengiriman Super Cepat', truck),
                new Service(1, 'Lacak dan Kelola Pesanan', location),
                new Service(2, 'Dukungan Pelanggan', headphones),
                new Service(3, 'Harga Bersaing', price)
            ].map((service) => (
                <li key={service.id}
